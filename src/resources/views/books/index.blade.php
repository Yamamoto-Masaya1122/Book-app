...
<form action="{{ route('books.search') }}" method="GET" class="flex rounded-lg border border-gray-300 h-24 items-center shadow-md">
  <div class="ml-8">
    <input type="text" name="title" class="w-full rounded-md border border-gray-300 p-1" placeholder="タイトル" />
  </div>
  <div class="ml-8">
    <input type="text" name="author" class="w-full rounded-md border border-gray-300 p-1" placeholder="著者" />
  </div>
  <div class="ml-8">
    <input type="text" name="publishing_company" class="w-full rounded-md border border-gray-300 p-1" placeholder="出版社" />
  </div>
  <div class="ml-8">
  ...
  <table class="w-full border-collapse bg-white text-left text-sm text-gray-500">
    <thead class="bg-gray-100">
      <tr>
        ...
        <th scope="col" class="px-6 py-4 font-medium text-gray-900">著者</th>
        <th scope="col" class="px-6 py-4 font-medium text-gray-900">出版社</th>
        <th scope="col" class="px-6 py-4 font-medium text-gray-900">価格</th>
    ...
    <tbody class="divide-y divide-gray-100 border-t border-gray-100">
      ...
      <tr class="hover:bg-gray-50">
        <th class="flex gap-3 px-6 py-4 font-normal text-gray-900">
          <div class="text-sm">
            <div class="font-medium text-gray-700">{{$book->id}}</div>
          </div>
        </th>
        ...
        <td class="px-6 py-4">
          <div class="flex gap-2">
            <span class="inline-flex items-center gap-1 rounded-full bg-blue-50 px-2 py-1 text-xs font-semibold text-blue-600"> {{$book->publishing_company}} </span>
          </div>
        </td>
        <td class="px-6 py-4">{{$book->price}}円</td>
        <td class="px-6 py-4">
          <div class="flex justify-end gap-4">
            ...
            <a x-data="{ tooltip: 'Edite' }" href="{{ route('books.edit', $book->id) }}">
